...
public class JavaScriptDatabase {
    /**
     * The file used to store all the data. This data takes the form of
     * JavaScript which can be evaluated to re-create the state of the
     * JavaScript environment in the database.
     */
     ...
     * The <code>tempFile</code> is used as a swap file, and may, under obscure
     * certain conditions, be the only living version of the data. If this
     * happens, the data will be loaded from the <code>tempFile</code> the next
	...
	RhinoUtil.evaluateURL(cx, scope,
		this.getClass().getResource(TurKit.js_libs_path + "/util.js"));
	...
	if (storageFile.exists()) {
	    ...
	    RhinoUtil.evaluateString(cx, scope, s,
		    storageFile.getAbsolutePath());

	    // save a copy of the storage file if it is corrupt
	    if (goodUntil < s.length()) {
		U.copyFile(storageFile,
			new File(storageFile.getAbsolutePath() + ".corrupt."
    ...
    /**
     * Releases resources associated with the JavaScript Database. In
     * particular, it releases a thread--failure to call <code>close</code> may
     * result in your program continuing to run after your main method has
     * ended.
     */
     ...
     * 
     * @param saveBackup
     *            set to true if you want to keep a backup copy of the database
     *            file
     */
	...
	public void run() {
		...
		while (true) {
			...
			if (currentTime >= nearestSaveTime) {
			    ...
			    JavaScriptDatabase.this.wait(nearestSaveTime
				    - currentTime);
			}
		    }
		}
    ...
    /**
     * Reformat the representation of the JavaScript database on disk to take up
     * less space.
     * 
     * @throws Exception
     */
    ...
    /**
     * Evaluates <code>q</code> as-is in the context of the JavaScript database,
     * and returns the resulting Rhino object.
     */
    ...
    /**
     * Evaluates <code>q</code> in the context of the JavaScript database. State
     * changes are persisted on disk, and the result is returned to the user
     * using {@link RhinoJson#json(Object)}.
     * 
     * <p>
     * NOTE: Queries are wrapped inside a function body, so a query of the form<br>
     * <code>var a = 5; return a</code><br>
     * becomes...<br>
     * <code>(function(){var a = 5; return a})()</code>
     * </p>
     * 
     * @param q
     * @return
